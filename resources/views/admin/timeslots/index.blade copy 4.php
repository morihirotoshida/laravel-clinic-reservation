            ...
            <table class="min-w-full bg-white">
                ...
                <tbody class="text-gray-700">
                    @forelse ($timeSlots as $slot)
                        ...
                        @if(\Carbon\Carbon::parse($slot->date . ' ' . $slot->start_time, 'Asia/Tokyo')->isFuture() || $slot->is_booked)
                        <tr>
                            <td class="text-left py-3 px-4">
                                {{-- 日付と時刻を結合し、日本のタイムゾーンとして解釈して表示します --}}
                                {{ \Carbon\Carbon::parse($slot->date . ' ' . $slot->start_time, 'Asia/Tokyo')->isoFormat('M/D(ddd) H:i') }}
                            </td>
                            <td class="text-left py-3 px-4">
                                @if($slot->is_booked)
