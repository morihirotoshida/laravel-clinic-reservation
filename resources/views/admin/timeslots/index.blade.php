                ...
                <div>
                    ...
                    <select id="start_time" name="start_time" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                        @php
                            ...
                            $end = \Carbon\Carbon::createFromTimeString('23:45');
                        @endphp
                        @while($start->lte($end))
        ...
        <div class="bg-white shadow-md rounded my-6 overflow-x-auto">
            <table class="min-w-full bg-white">
                ...
                <tbody class="text-gray-700">
                    {{-- 管理者ページでは全ての時間枠を表示するため、@ifによる条件分岐を削除しました --}}
                    @forelse ($timeSlots as $slot)
                        <tr>
                            <td class="text-left py-3 px-4">
                                {{-- ↓↓↓ 正しいフォーマット文字に修正しました ↓↓↓ --}}
                                {{ \Carbon\Carbon::parse($slot->date . ' ' . $slot->start_time, 'Asia/Tokyo')->format('n/j(D) H:i') }}
                            </td>
                            <td class="text-left py-3 px-4">
                                @if($slot->is_booked)
